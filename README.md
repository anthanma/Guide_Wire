...
- [The Vision](#the-vision-why-omniguard-exists)
- [The Phase 1 Crisis](#the-phase-1-crisis-the-market-crash)
...
If an honest worker loses their network in the storm, the app switches to **offline-first mode**. It locally encrypts their hardware telemetry (barometric drops, erratic steps through water). Upon reconnecting, this **"black box" data** authenticates their struggle and automatically releases the escrowed funds.

### 🏅 Trust Tier System

Not all flagged workers are treated equally. Workers with **6+ months of clean history** and consistent telemetry are automatically granted **Tier-1 status**. If flagged, their escrow releases automatically without any manual review. New or low-history workers go through a lightweight review process. Fairness is proportional to reputation.
...
| 🔧 Sensor Fallback | Graceful degradation to kinematic/network layers | Budget phones without barometers |
| 📡 Network Variance | BSSID fluctuation + cell sector corroboration | Static emulator environments |
| 🔋 Thermal/Battery | Non-linear discharge & heat signature analysis | Server farm emulators |
| 💰 Escrow State | Pending funds, never auto-denied | Protects honest workers |
| 🔐 Offline Cache | Encrypted local telemetry black box | Protects workers in dead zones |
